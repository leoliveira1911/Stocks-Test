...
export default function useWallet() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    const form = {
      ...
      ticker,
      profitPercent: `=ROUND((G${values.rowCount + 2}-F${values.rowCount +
        2})/G${values.rowCount + 2}*(-100);2)`,
      profitAbsolute: `=FIXED((F${values.rowCount + 2}-G${values.rowCount +
        2})*H${values.rowCount + 2};2)`,
      price: `=FIXED(GOOGLEFINANCE(C${values.rowCount + 2}) ;2 )`,
      buyPrice: `=FIXED(${buyPrice};2)`,
      shares,
      investedValue: `=FIXED(H${values.rowCount + 2}*G${values.rowCount +
        2}; 2)`,
